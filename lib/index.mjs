...
var SwitchWatermarkSync = class {
	...
	constructor(engine, sessionQuery, readArchiveSource) {
		this.engine = engine;
...
*/
async function rebuildIndex(activeEngine, layout, sessionQuery, keepArchives, onProgress, archiveSource) {
	const state = {
		state: "building",
		done: 0,
		total: 0,
...
async function indexRebuild(runtime) {
	...
	}, keepArchives, void 0, runtime.registry).then((state) => {
		index.rebuild = state;
	}).catch((err) => {
		index.rebuild = {
			state: "error",
...
async function listArchived(runtime) {
	const index = runtime.index;
	if (index.engine.isOpen === false) return {
		ok: false,
		error: "独立索引未就绪"
