...
async function openIndexDatabase(path) {
	...
	try {
		...
		if (applicationId === 1398229332 && version !== 2) resetSchema(db);
...
function sanitizeFtsQuery(query) {
	return query.split(/\s+/u).filter((term) => [...term].length >= 3).map((term) => `"${term.replace(/"/g, "\"\"")}"`).join(" ");
}
...
var SwitchIndexEngine = class {
	...
	upsertSession(input) {
		...
		withTransaction(db, () => {
			db.prepare("DELETE FROM docs_fts WHERE doc_id IN (SELECT doc_id FROM docs WHERE session_id = ?)").run(input.sessionId);
	...
	*/
	...
	removeSession(sessionId) {
		const db = this.requireDb();
		withTransaction(db, () => {
			db.prepare("DELETE FROM docs_fts WHERE doc_id IN (SELECT doc_id FROM docs WHERE session_id = ?)").run(sessionId);
	...
	*/
	search(request) {
        ...
        ORDER BY ftsRank
        LIMIT ?
      `).all(match, MATCH_SCAN_LIMIT);
			if (rows.length === 0) return [];
			const rankByDocId = new Map(rows.map((row) => [Number(row.docId), Number(row.ftsRank)]));
      ...
      `).all(...docIds, ...types);
			for (const doc of fetched) docs.push({
				...doc,
				rank: rankByDocId.get(Number(doc.docId)) ?? 0
			});
		} else {
        ...
        LIMIT ?
      `).all(...types, ...patterns, fallbackLimit);
			for (const doc of fetched) docs.push({
				...doc,
				rank: 0
			});
		...
		return [...bestBySession.values()].sort((a, b) => b.score - a.score).slice(0, limit).map(({ doc }) => ({
...
var SwitchWatermarkSync = class {
	...
	constructor(engine, sessionQuery) {
		this.engine = engine;
...
*/
async function rebuildIndex(activeEngine, layout, sessionQuery, keepArchives, onProgress) {
	const state = {
		state: "building",
		done: 0,
		total: 0,
...
const name = "dsh-session-search-toggle";
...
const Config = z.object({
	...
	syncIntervalMs: z.number().default(3e4),
...
async function listSessions(ctx) {
	...
	if (sessionQuery === void 0) {
		if (index?.engine.isOpen === true) return {
			ok: true,
...
async function contentSearch(ctx, payload) {
	...
	const index = getIndex(ctx);
	if (index === void 0 || index.engine.isOpen === false) return {
		ok: false,
...
*/
async function indexRebuild(ctx) {
	const index = getIndex(ctx);
	if (index === void 0) return {
		ok: false,
		error: "索引服务未就绪"
...
async function indexExport(ctx, res) {
	const index = getIndex(ctx);
	if (index === void 0 || index.engine.isOpen === false) {
		writeJson(res, 200, {
			ok: false,
			error: "独立索引未就绪"
		});
...
async function indexImport(ctx, text) {
	const index = getIndex(ctx);
	if (index === void 0) return {
		ok: false,
		error: "索引服务未就绪"
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (method === "index-import") {
					writeJson(res, 200, await indexImport(ctx, await readRawBody(req)));
					return;
				}
...
//#endregion
export { Config, DEFAULT_CONFIG, DEFAULT_INDEX_LAYOUT, SWITCH_SEARCH_SETTINGS_NAMESPACE, SwitchIndexEngine, SwitchWatermarkSync, apply, exportSnapshot, importIntoIndex, inject, name, parseSnapshot, rebuildIndex };
