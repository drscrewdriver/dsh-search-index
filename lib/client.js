window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** ------------------------------------------------------------------ styles */
		const CSS = `
.dsws_root{box-sizing:border-box;position:relative;display:flex;align-items:center;flex:1 1 auto;min-width:0}
.dsws_rootRail{flex:none}
...
.dsws_buttonRail{flex:none;width:36px;height:36px;padding:0;gap:0;justify-content:center;border-radius:50%}
...
`;
		...
		*/
		...
		const SETTINGS_CARD_SEAT = "settings.plugin.item";
		/**
		...
		*/
		function apply(ctx) {
			...
			const entryScope = ctx.get("settingsScope")?.bind({ namespace: SWITCH_SEARCH_SETTINGS_NAMESPACE });
			...
			const tabTitle = typeof locale?.bind === "function" ? locale.bind(NS) : void 0;
			const cardInject = () => ({
				scope: entryScope ?? {
					getSnapshot: () => ({
						status: "ready",
					...
					slots.inject(slotName, () => {
						try {
							return slots.register({
								...
								order: isTab ? 100 : void 0,
								locale: locale !== void 0 ? NS : void 0,
								label: isTab ? () => tabTitle !== void 0 ? tabTitle("card.title") : zh["card.title"] : void 0,
								inject: cardInject
							}, SearchSettingsCard);
						} catch (err) {
			...
			setTimeout(() => {
				...
				console.warn(`[dsh-search-index] 宿主未声明 ${SETTINGS_CARD_SEAT}：设置里的「搜索索引」卡片不会出现。（0.1.5 的座位集合本机未验证，请在真机上确认。）`);
			}, SEAT_PROBE_MS);
		}
		//#endregion
		exports.SETTINGS_CARD_SEAT = SETTINGS_CARD_SEAT;
